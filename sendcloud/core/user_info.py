...
from ..conf import get_user_info_url, api_user_add_url, api_user_list_url
...
class UserInfoAPI(SendCloudAPIBase):
    ...
    def api_user_list(self, **kwargs):
        ...
        if _email_type:
            _data.update({"emailType": _email_type})

        if _ctype:
            _data.update({"cType": _ctype})

        if _domain:
            _data.update({"domainName": _domain})
